...
function GRAM_GeodesicRegistration(dirBin,dirSubject,dirFinalField,gpath,gmean,NNiter,NNsmoothing)
    ...
    if ~exist([dirFinalField,'/',Fix,'to',f(s).name(1:end-4),'-def.nii'],'file') || s == gmean
        ...
        elseif length(sPath) >= 3
            ...
            for i = length(sPath):-1:3
                ...
                cmd = [dirBin,'/ConcatenateFields ',FixToInter,' ',InterToMov,' ',FixToMov];
                dos(cmd);
