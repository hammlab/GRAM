 ...
 */
 ...
 /*
   Modified by Dong Hye Ye for speed up and nearest neighbor interpoloation
 */
...
*/
  ...
  if ( args.verbosity > 0 && !args.outputImageFile.empty() )
    {
    typedef itk::DisplacementFieldJacobianDeterminantFilter
	...
	std::basic_string<char>::size_type pos = args.outputImageFile.rfind( "." );

	writer->SetFileName( args.outputImageFile.replace(pos, 4, "-jac.nii").c_str() );
	...
	const double harmonicEnergy
         = HarmonicEnergyCalculator->GetHarmonicEnergy();
